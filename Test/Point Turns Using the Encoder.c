...
/*
  ...
  while(abs(SensorValue(quadrature_right)) < 1800)   //While quadrature_right has counted less than 1800 counts
	{
		//Turn Left
		motor[right_motor] = 100;					      //Motor on port2 is run at full (127) power forward
		motor[left_motor]  = -127;				        //Motor on port3 is run at full (-127) power reverse
	}
*/
	SensorValue[quadrature_left] = 0;	            //Set the encoder so that it starts counting at 0

  while(abs(SensorValue(quadrature_left)) < 1800)    //While quadrature_left has counted less than 1800 counts
	{
		//Turn Right
		motor[right_motor] = -100;				        //Motor on port2 is run at full (-127) power reverse
